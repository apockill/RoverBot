...
class FollowLine:
    ...
    def moveTowards(self, point):
        ...
        speed  = (self.targetSpeed * .6)  # clamp(self.targetSpeed * yMag, self.targetSpeed*.5, self.targetSpeed)
        lSpeed = self.targetSpeed*.3 + speed * xMag  # Where -1 xmag will lower left turning speed
        rSpeed = self.targetSpeed*.3 - speed * xMag  # Where -1 xmag will raise the right turning speed
